...
const ResetPassword = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (currentUser !== null) {
                ...
                setLoading(true)
                const request = await axios.post(`${api}/auth/reset-password`, data, {
                    headers: {
                ...
                setLoading(false)
                if (!response.isError) setMessage({
                    error: false,
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}
            className="space-y-6 lg:space-y-8 lg:px-6 mt-10 [&_small]:text-red-500 [&_small]:italic">
            <div>
            ...
            <div>
                <button
                    type="submit"
                    disabled={loading}
                    className="flex justify-center px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent 
                    rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 items-center"
                >
                    {loading ? (
                        <>
                ...
                <div>
                    <p className={`font-medium ${message.error ? "text-red-500" : "text-green-500"}`}>{message.msg}</p>
                </div>
            </div>
        </form>
    )
